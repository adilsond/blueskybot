...
#Based on https://github.com/bluesky-social/atproto-website/blob/main/examples/create_bsky_post.py
...
def is_api_json_valid():
    # Verifica se o arquivo "api.json" existe
    if not os.path.exists("apig1.json"):
        return False
    
    # Obtém a data de modificação do arquivo
    modification_time = os.path.getmtime("apig1.json")
    ...
    # Verifica se o arquivo expirou (1 hora de validade)
...
def bsky_login_session(pds_url: str, handle: str, password: str) -> Dict:
    if not os.path.exists("apig1.json") or os.path.getsize("apig1.json") == 0:
        # Se o arquivo "apig1.json" não existir ou estiver vazio, execute a solicitação para obter os dados
        resp = requests.post(
            pds_url + "/xrpc/com.atproto.server.createSession",
            json={"identifier": handle, "password": password},
        )
        resp.raise_for_status()
        resp_json = resp.json()  # Obtenha os dados JSON da resposta da solicitação
        with open("apig1.json", "w") as file:
            json.dump(resp_json, file)  # Escreva os dados JSON no arquivo
        return resp_json
    else:
        # Se o arquivo "apig1.json" existir e não estiver vazio, carregue os dados do arquivo
        with open("apig1.json", "r") as file:
            resp_json = json.load(file)  # Carregue os dados JSON em uma variável
            return resp_json  # Retorne os dados JSON carregados
...
def fetch_embed_url_card(pds_url: str, access_token: str, url: str) -> Dict:
    ...
    if image_tag:
        ...
        # Check if the image size exceeds 976KB
        if len(resp.content) > 976 * 1024:
            # Resize the image to fit within 970KB
            max_size_bytes = 970 * 1024
            ...
            output = io.BytesIO()
            img.save(output, format="JPEG")  # You can specify the desired format
            card["thumb"] = upload_file(pds_url, access_token, img_url, output.getvalue())
        else:
...
def create_post(args):
    ...
    posttext = re.sub(r"^(?:')(.*?)(?:')$", r"\1", args.text)
    #Now posts with posttext processed
    post = {   
...
def main():
    ...
    parser.add_argument("encoded_text", default="")
    ...
    if args.image and len(args.image) > 4:
        ...
        sys.exit(-1)
    decoded_text = base64.b64decode(args.encoded_text).decode('utf-8')
    args.text = decoded_text
    create_post(args)
